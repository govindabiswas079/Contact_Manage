...
export const createContact = (form) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post('http://localhost:5000/contact', form, config);
...
export const fetchContacts = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get('http://localhost:5000/contact', config);
...
export const deleteContact = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`http://localhost:5000/contact/${id}`, config);
...
export const deleteContacts = (idArr) => async (dispatch, getState) => {
  try {
    ...
    await axios.post(`http://localhost:5000/contact/delete`, idArr, config);
...
export const updateContact = (id, form) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(`http://localhost:5000/contact/${id}`, form, config);
    console.log(data);
